...
const reactToPrintFn = useReactToPrint({
  contentRef, documentTitle: resume.title || "Resume"
    ...
    return <div className="group relative rounded-lg border border-transparent bg-secondary p-3 transition-colors hover:border-border">
  <div className="space-y-3">
    <Link 
    href={`/editor?resumeId=${resume.id}`}
    className="inline-block w-full text-center">
      <p className="font-semibold line-clamp-1">{resume.title || "No Title"}</p> {resume.description &&(
    ...
    > <ResumePreview
    resumeData={mapToResumeValues(resume)} contentRef={contentRef} classname="overflow-hidden shadow-sm group-hover:shadow-lg transition-shadow"
    /> </Link>
  </div>
...
interface MoreMenuProps {
  resumeId: string; onPrintClick: () =>void;
}
...
interface DeleteConfirmationDialogProps{
  resumeId: string; open: boolean; onOpenChange: (open:boolean) => void;
}
